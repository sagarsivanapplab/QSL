...
(function(factory) {
        ...
        function EvoCalendar(element, settings) {
            ...
            _.initials = {
                ...
                dates: {
                    ...
                    ar: {
                        days: ["الأحد", "الإثنين", "الثلاثاء", "الأربعاء", "الخميس", "يوم الجمعة", "يوم السبت"],
    ...
    EvoCalendar.prototype.buildTheBones = function() {
        ...
        if (!_.$elements.calendarEl.html()) {
            ...
            markup += '<div class="calendar-events">'+
                            '<div class="event-header">'+events_on+'&nbsp;<p></p></div>'+
    ...
    EvoCalendar.prototype.buildEventList = function() {
        ...
        if(!hasEventToday) {
            if(_.options.language == 'en') {
                markup = '<div class="no-data"><div class="no-data__icon"></div><p>No events available this day</p></div>';
            }
            else {
                markup = '<div class="no-data"><div class="no-data__icon"></div><p>لا يوجد فعاليات في  هذا اليوم</p></div>';
            }
        }
    ...
    EvoCalendar.prototype.removeEventList = function(event_data) {
        ...
        if (eventListEl.find('[data-event-index]').length === 0) {
            ...
            if (_.$active.date === _.$current.date) {
                if(_.options.language == 'en') {
                    markup = '<p>No event for today..</p>';
                }
                else {
                    markup = '<p>لا يوجد فعاليات اليوم..</p>';
                }
            } else {
                if(_.options.language == 'en') {
                    markup = '<p>No event for this day..</p>';
                }
                else {
                    markup = '<p>لا يوجد فعاليات لهذا اليوم..</p>';
                }
            }
            eventListEl.append(markup)
        }
    ...
    EvoCalendar.prototype.removeEventIndicator = function(active_date, type) {
        ...
        if (eventLength === 0) {
            _.$elements.innerEl.find('[data-date-val="'+active_date+'"] span.event-indicator > .type-bullet > .type-'+type).parents('span.event-indicator').remove();
        }
    }
